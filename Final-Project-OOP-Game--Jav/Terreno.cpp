...
void Terreno::mejorarTerreno(Tipo nuevoTipo, Nivel nuevoNivel)
{
    // 1. Actualiza el Tipo
    this->capacidadTipo = nuevoTipo;

    // 2. Añade el nuevo nivel permitido [cite: 37]
    this->nivelPermitidoA.push_back(nuevoNivel);

    // 3. (Opcional) Actualiza las capacidades
    if (nuevoTipo == Tipo::RANCHO_M) {
        this->capacidadEspecie = 3; // (Como ya estaba [cite: 12])
        this->capacidadStack = 4; // (Como ya estaba [cite: 12])
    } else if (nuevoTipo == Tipo::FINCA_M) {
        this->capacidadEspecie = 4; // (Tu diagrama dice 4-3? [cite: 14])
        this->capacidadStack = 9; // (Tu diagrama dice 9 [cite: 14])
    }
}
